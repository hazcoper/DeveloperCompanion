...
if __name__ == '__main__':
    ...
    virtual_keys.directKey("a")
    sleep(0.04)

    virtual_keys.directKey("a", virtual_keys.key_release)
    
    # queue of keys (direct keys, threaded, only for keybord input)
    virtual_keys.parseKeyString("a_down,-4,a_up,0x01")  # -4 - pause for 4 ms, 0x00 - hex code of Esc
    
    # queue of keys (virtual keys, threaded, only for keybord input)
    virtual_keys.parseKeyString("vk,a_down,-4,a_up")  # -4 - pause for 4 ms
